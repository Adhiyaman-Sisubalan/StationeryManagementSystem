...
namespace AD_Project.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        /*public ActionResult PurchaseOrder()
        {
            ...
            var ItemsSupplier = _context.Items.Include(c => c.Stockdetails).Include(c => c.Suppliers_Prices).ToList();


            List<Item> it = new List<Item>();
            foreach (var x in ItemsSupplier)
            {
                foreach (var j in x.Stockdetails)
                {
                    foreach (var q in x.Suppliers_Prices)
                    {
                        if (j.InventoryStockQty < x.ReorderLevel && j.ItemId == x.Id)
                        {
                            it.Add(x);
        ...
        public ActionResult SpecifySupplier(PurchaseOrderViewModel purchaseOrderViewModel)
        {
           ...
           /* var ItemsSupplier = _context.Items.Include(c => c.Suppliers_Prices).Include(c => c.Stockdetails)
                .SingleOrDefault(m => m.SupplierPriceId == purchaseOrderViewModel.SelectedsupplierId);*/
            ...
            try
            {
               // purchaseOrderViewModel.iitems = items;
